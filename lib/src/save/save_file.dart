...
class Save {
  ...
  _setSheetElements() {
    ...
    _excel._sheetMap.forEach((sheet, value) {
      ...
      var dvs = _excel._sheets[sheet].findElements('dataValidations');
      XmlElement xe = null;
      if (dvs.isNotEmpty) {
        xe = dvs.first;
      } else {
        xe = XmlElement(XmlName('dataValidations'));
        _excel._sheets[sheet].children.add(xe);
      }

      //       String type = node.getAttribute('type');
      // String range = node.getAttribute('sqref').toString();
      value._dataValidations.forEach((dv) {
        var nxe = XmlElement(XmlName('dataValidation'), [XmlAttribute(XmlName('type'), dv.type), XmlAttribute(XmlName('sqref'), dv.range)]);
        nxe.innerText = dv.value;
        xe.children.add(nxe);
      });
    });
  }
  ...
  _setRTL() {
    this._excel._rtlChangeLook.forEach((s) {
      ...
      if (_isContain(sheetObject) && this._excel._xmlSheetId.containsKey(s) && this._excel._xmlFiles.containsKey(this._excel._xmlSheetId[s])) {
        var itrSheetViewsRTLElement = this._excel._xmlFiles[_excel._xmlSheetId[s]].findAllElements('sheetViews');

        if (itrSheetViewsRTLElement.isNotEmpty) {
          var itrSheetViewRTLElement = this._excel._xmlFiles[_excel._xmlSheetId[s]].findAllElements('sheetView');

          if (itrSheetViewRTLElement.isNotEmpty) {
            /// clear all the children of the sheetViews here
            this._excel._xmlFiles[_excel._xmlSheetId[s]].findAllElements('sheetViews')?.first?.children?.clear();
          }
          this._excel._xmlFiles[_excel._xmlSheetId[s]].findAllElements('sheetViews').first.children.add(XmlElement(
                XmlName('sheetView'),
                [
          ...
          this._excel._xmlFiles[_excel._xmlSheetId[s]].findAllElements('worksheet').first.children.add(XmlElement(XmlName('sheetViews'), [], [
                XmlElement(
                  XmlName('sheetView'),
                  [
                    if (sheetObject.isRTL) XmlAttribute(XmlName('rightToLeft'), '1'),
  ...
  _processStylesFile() {
    ...
    _innerCellStyle.forEach((cellStyle) {
      _FontStyle _fs = _FontStyle(
          bold: cellStyle.isBold, italic: cellStyle.isItalic, fontColorHex: cellStyle.fontColor, underline: cellStyle.underline, fontSize: cellStyle.fontSize, fontFamily: cellStyle.fontFamily);
    ...
    innerFontStyle.forEach((fontStyleElement) {
      fonts.children.add(XmlElement(XmlName('font'), [], [
        ...
        if (fontStyleElement.underline != null && fontStyleElement.underline != Underline.None && fontStyleElement.underline != Underline.Single && fontStyleElement.underline == Underline.Double)
          XmlElement(XmlName('u'), [XmlAttribute(XmlName('val'), 'double')], []),

        /// putting fontFamily
        if (fontStyleElement.fontFamily != null && fontStyleElement.fontFamily.toLowerCase().toString() != "null" && fontStyleElement.fontFamily != "" && fontStyleElement.fontFamily.isNotEmpty)
    ...
    _innerCellStyle.forEach((cellStyle) {
      ...
      if (horizontalALign != HorizontalAlign.Left || textWrapping != null || verticalAlign != VerticalAlign.Bottom || rotation != null || rotation != 0) {
